...
public class QuestionsDaoCsvResources implements QuestionsDao {
    ...
    public QuestionsDaoCsvResources(DaoProperties daoProperties) {
        this.resourceName = daoProperties.getCsvResourceName();
        this.resultFileName = daoProperties.getResultsFileName();
    }
    ...
    public List<Question> getQuestions() throws Exception {
        ...
        try (var csvReader = new CSVReader(new InputStreamReader(getClass().getResourceAsStream(localizedName)))) {
             ...
             while ((line = csvReader.readNext()) != null) {
                 if (line.length < 4)
                     throw new DaoException("Expected in csv-line: a question, an index of correct variant, from 2 to 6 variants.");
